...
export class SeaModel {
	...
	static geometry = new Three.PlaneBufferGeometry(100, 100, 1000, 1000);
	...
	geometry1 = new Three.PlaneGeometry(100, 100);
	...
	plane = new Three.Mesh(this.geometry1, this.material1);
	init(scene) {
	...
	run(boatModel, state, coneModel) {
		...
		const y = SeaModel.calcY(zB, xB, clk) - 0.1 + 0.05 * Math.cos(clk*2);
	...
	makeWaves(Intersection, state, z, x) {
		...
		vec41.z = vec41.x + state.linearVelocity.intensity * Maths.cos(angle + 160);
		vec41.w = vec41.y + state.linearVelocity.intensity * Maths.sin(angle + 160);

		vec42.x = vec41.x;
		vec42.y = vec41.y;
		vec42.z = vec42.x + state.linearVelocity.intensity * Maths.cos(angle - 160);
		vec42.w = vec42.y + state.linearVelocity.intensity * Maths.sin(angle - 160);
